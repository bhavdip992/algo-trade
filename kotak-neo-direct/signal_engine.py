"""
signal_engine.py  v2.0
Pure pandas/numpy — no ta-lib, no external dependencies.
FIX: warming_up now based on slow_len (50) not trend_len (200).
"""
...
class EngineConfig:
    ...
    # Trail
    use_trail:         bool  = True
    trail_trig:        float = 1.2
    trail_step:        float = 0.6
    # Sizing
...
class SignalEngine:
    ...
    def update(self, timestamp, open_, high, low, close, volume) -> SignalResult:
        ...
        # FIX: avoid FutureWarning from concat with empty DataFrame
        if self.df.empty:
    ...
    def _evaluate(self) -> SignalResult:
        ...
        # FIX: only need slow_len bars, not trend_len (200)
        ...
        # EMA trend — only use if enough bars
        et  = float(_ema(c, min(cfg.trend_len, n-1)).iloc[-1]) if n > 10 else float(_ema(c, cfg.slow_len).iloc[-1])
        ...
        # Conditions
        cross_up   = ef1 < em1 and ef >= em
        cross_dn   = ef1 > em1 and ef <= em
        ...
        ce_flags = [cross_up,  bull_trend, st_bull, vwap_bull, rsi_buy,  trending, vol_spike, no_squeeze]
        pe_flags = [cross_dn,  bear_trend, st_bear, vwap_bear, rsi_sell, trending, vol_spike, no_squeeze]

        # Volume is always optional for index feed
        if not cfg.vol_required:
            ce_cnt = sum(ce_flags[:6]) + sum(ce_flags[7:])   # skip index 6
            pe_cnt = sum(pe_flags[:6]) + sum(pe_flags[7:])
            min_r  = cfg.min_confirmations - 1
        else:
            ce_cnt = sum(ce_flags)
            pe_cnt = sum(pe_flags)
            min_r  = cfg.min_confirmations

        buy_ok  = ce_cnt >= min_r and cfg.option_type != "PE Only"
        sell_ok = pe_cnt >= min_r and cfg.option_type != "CE Only"
        if buy_ok and sell_ok:
            buy_ok  = ce_cnt >= pe_cnt
            sell_ok = not buy_ok

        signal = "BUY_CE" if buy_ok else "BUY_PE" if sell_ok else "NONE"

        names = ["EMAxross","Trend","SuperTrend","VWAP","RSI","ADX","Volume","NoBBsq"]
        if signal == "BUY_CE":
            fired  = [names[i] for i, f in enumerate(ce_flags) if f]
            reason = f"CE {ce_cnt}/{min_r}: {', '.join(fired)}"
        elif signal == "BUY_PE":
            fired  = [names[i] for i, f in enumerate(pe_flags) if f]
            reason = f"PE {pe_cnt}/{min_r}: {', '.join(fired)}"
        else:
            mc = [names[i] for i, f in enumerate(ce_flags) if not f]
            mp = [names[i] for i, f in enumerate(pe_flags) if not f]
            reason = f"NONE | CE needs: {','.join(mc[:3])} | PE needs: {','.join(mp[:3])}"
        ...
        return SignalResult(
            ...
            conf_ema_cross=cross_up or cross_dn,  conf_trend=bull_trend or bear_trend,
